...
export const createYDoc = ({
  ...
  const mySharedMap = doc.getMap<string>("thing");
...
export const updateThing = (key: string, value: string) => {
...
export const deleteThing = (key: string) => {
  if (!docStore.sharedMap) {
    console.warn("Shared map is not initialized.");
    return;
  }
